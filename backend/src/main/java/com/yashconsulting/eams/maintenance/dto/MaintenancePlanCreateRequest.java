...
@AllArgsConstructor
@Builder
@Schema(description = "Request body payload to create a new preventive maintenance plan")
public class MaintenancePlanCreateRequest {

    @Schema(description = "ID of the asset associated with the maintenance plan", example = "1", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotNull(message = "Asset ID must not be null")
    private Long assetId;

    @Schema(description = "Unique code identifying the maintenance plan", example = "MP-HVAC-001", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "Plan code must not be blank")
    @Size(max = 100, message = "Plan code must not exceed 100 characters")
    @NoLeadingTrailingWhitespace(message = "Plan code must not contain leading or trailing spaces")
    ...
    @Schema(description = "Initial status of the maintenance plan (default: SCHEDULED)", example = "SCHEDULED")
    @Builder.Default
