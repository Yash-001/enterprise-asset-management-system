...
@AllArgsConstructor
@Builder
@Schema(description = "Request body payload to create a new spare part")
public class SparePartCreateRequest {

    @Schema(description = "Unique alphanumeric identifier/SKU for the spare part", example = "SP-HVAC-BELT", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "Part number must not be blank")
    @Size(max = 100, message = "Part number must not exceed 100 characters")
    @NoLeadingTrailingWhitespace(message = "Part number must not contain leading or trailing spaces")
